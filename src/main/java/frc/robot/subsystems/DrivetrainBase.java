...
public class DrivetrainBase extends SubsystemBase {
  protected static WPI_VictorSPX leftMas  = new WPI_VictorSPX(Chassis.leftMaster);
  protected static WPI_VictorSPX leftFol  = new WPI_VictorSPX(Chassis.leftFollewer);
  protected static WPI_VictorSPX rightMas = new WPI_VictorSPX(Chassis.rightMaster);
  protected static WPI_VictorSPX rightFol = new WPI_VictorSPX(Chassis.rightFollower);
  ...
  public void firstConfig(){
    ...
    leftFol.getStatusFramePeriod(StatusFrameEnhanced.Status_1_General, 255);
    rightFol.getStatusFramePeriod(StatusFrameEnhanced.Status_1_General, 255);
    leftMas.setNeutralMode(NeutralMode.Coast);
    rightMas.setNeutralMode(NeutralMode.Coast);
  }
  public boolean isMove(){
    if(Math.abs(leftMas.getSelectedSensorVelocity()) > 2160){
      return true;
    }
    return false;
  }
